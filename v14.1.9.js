...
function hnFbListenPosts() {
  ...
  hn_fb_ref('hunternet/posts').on('value', function(snap) {
    ...
    console.log('[FB-LISTENER] posts count='+count+' stack='+(new Error().stack.split('\n')[2]||'').trim());
    if (count === 0 && hn_data_ready) {
      console.warn('[FB-LISTENER] WARNING: received empty posts after data was ready! Ignoring.');
      return;
    }
    if (d) {
...
function hnFbListenProfiles() {
  ...
  hn_fb_ref('hunternet/profiles').on('value', function(snap) {
    ...
    hnRenderSidebar();
    if (hn_posts.length > 0) hnRenderFeed();
  });
}
...
function hnRenderFeed() {
  hnFixLayout();
  var list = document.getElementById('hn-posts');
  if (!hn_data_ready && hn_posts.length === 0) {
