...
function hnSubmitPost() {
  ...
  var post = { id:String(Date.now()), authorId:hn_user.id, authorName:hn_user.name,
    ...
    likes:[], dislikes:[], comments:[], poll:poll_data, linkUrl:link_url||'', linkTitle:link_title||'', createdAt:new Date().toISOString() };
  hn_posts.unshift(post);
  hnSavePost_fb(post,
    function(){ ta.value=''; hnClearImg(); hnUpdateChar(); btn.disabled=false; btn.textContent='Pubblica'; hnNotifMentions(text, post.id, text); hnNotifGroupTags(text, post.id, text); hn_poll_options=[]; hn_poll_deadline_val=''; var pc=document.getElementById('hn-poll-compose'); if(pc)pc.style['display']='none'; hn_link_url=''; hn_link_title=''; var lr=document.getElementById('hn-linkurl-row'); if(lr)lr.style['display']='none'; var li2=document.getElementById('hn-linkurl'); if(li2)li2.value=''; var lt2=document.getElementById('hn-linktitle'); if(lt2)lt2.value=''; },
...
function hnRenderPost(p) {
  ...
  return '<div class="hn-post" id="hn-p-'+p.id+'"><div class="hn-phead">'+
    ...
    '<div class="hn-ptext" id="hn-pt-'+p.id+'">'+th+'</div>'+imgHtml+linkHtml+sharedHtml+pollHtml+
