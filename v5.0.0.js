...
function hnVerifiedBadge(prof) {
  ...
  return '<span class="mdi mdi-check-decagram" style="color:#5b9cf6!important;font-size:14px!important;margin-left:3px!important;vertical-align:middle!important;" title="Utente verificato"></span>';
}
...
function hnRenderPost(p) {
  ...
  var th = p.text.replace(/(#\w+)/g,'<span class="hn-hi">$1</span>');
  th = hnMentionify(th);
  ...
  return '<div class="hn-post" id="hn-p-'+p.id+'"><div class="hn-phead">'+
    ...
    '<span class="hn-pname" onclick="hnOpenProfile(\''+p.authorId+'\')">'+hnEsc(p.authorName)+'</span>'+
...
function hnRenderHunters() {
  ...
  for (var j=0;j<recent.length;j++) {
    ...
    html+='<div class="hn-hunter" onclick="hnOpenProfile(\''+r.authorId+'\')">'+
      ...
      '<div style="margin:0!important;text-align:left!important;"><div class="hn-hname">'+hnEsc(r.authorName)+'</div><div class="hn-htag2">@'+hnEsc(r.authorHandle)+'</div></div></div>';
  }
  el.innerHTML=html;
}
