...
function hnSubmitPost() {
  ...
  var post = { id:String(Date.now()), authorId:hn_user.id, authorName:hn_user.name,
    ...
    likes:[], dislikes:[], comments:[], poll:poll_data, createdAt:new Date().toISOString() };
  hn_posts.unshift(post);
  hnPUT(hn_api+'/'+hn_bin_post, {posts:hn_posts},
    function(){ ta.value=''; hnClearImg(); hnUpdateChar(); hnRenderFeed(); hnRenderSidebar(); btn.disabled=false; btn.textContent='Pubblica'; hnNotifMentions(text, post.id, text); hnNotifGroupTags(text, post.id, text); hn_poll_options=[]; hn_poll_deadline_val=''; var pc=document.getElementById('hn-poll-compose'); if(pc)pc.style['display']='none'; },
...
function hnRenderPost(p) {
  ...
  if (author_prof) {
    ...
    online_dot_html = '<span style="position:absolute!important;bottom:0!important;right:0!important;width:9px!important;height:9px!important;border-radius:50%!important;background:'+dot_color+'!important;border:2px solid #252932!important;display:block!important;"></span>';
  }
  return '<div class="hn-post" id="hn-p-'+p.id+'"><div class="hn-phead">'+
    ...
    '<div class="hn-ptext" id="hn-pt-'+p.id+'">'+th+'</div>'+imgHtml+sharedHtml+pollHtml+
    ...
    (hn_user ? '<button class="'+(disliked?'hn-abtn hn-disliked':'hn-abtn')+'" onclick="hnToggleDislike(\''+p.id+'\')">'+'<svg viewBox="0 0 24 24" fill="'+(disliked?'currentColor':'none')+'" stroke="currentColor" stroke-width="2"><path d="M10 15v4a3 3 0 0 0 3 3l4-9V2H5.72a2 2 0 0 0-2 1.7l-1.38 9a2 2 0 0 0 2 2.3H10z"/><path d="M17 2h2.67A2.31 2.31 0 0 1 22 4v7a2.31 2.31 0 0 1-2.33 2H17"/></svg>'+dc+'</button>' : '')+
