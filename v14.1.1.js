...
function hnCheckPollExpiryRun() {
  ...
  if (changed) hnSavePosts(function(){hnRenderFeed();},function(){});
}
/* ── FIREBASE CONFIG ── */
...
function hnVotePoll(pid, opt_idx) {
  ...
  hnSavePosts(function(){hnRenderFeed();},function(){alert('Errore nel voto.');});
}
...
function hnSavePost(pid) {
  ...
  hnSavePosts( function(){ hnRenderFeed(); }, function(){ alert('Errore nel salvataggio.'); hnRenderFeed(); });
}
function hnDeletePost(pid) {
  if (!confirm('Eliminare questo post?')) return;
  for (var i = 0; i < hn_posts.length; i++) { if (hn_posts[i].id === pid) { hn_posts.splice(i,1); break; } }
  hnSavePosts( function(){ hnRenderFeed(); hnRenderSidebar(); }, function(){ alert('Errore nell\'eliminazione.'); hnRenderFeed(); });
}
