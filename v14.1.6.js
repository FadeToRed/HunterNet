...
function hnSearch(query) {
  ...
  if (!query) { hn_search_active = false; hnRenderFeed(); hnRenderSearchUsers([]); return; }
...
function hnFbListenPosts() {
  ...
  hn_fb_ref('hunternet/posts').on('value', function(snap) {
    var d = snap.val();
    console.log('[listener] snap keys:', d ? Object.keys(d).length : 0);
    if (d) {
...
function hnRestoreSession() {
  ...
  if (!s) return;
  try { hn_user = JSON.parse(s); hn_fb_get('hunternet/notifications/'+hn_user.id, function(d){ hn_notifs = d ? Object.values(d) : []; hnUpdateBadge(); }, function(){}); hnFbListenNotifs(hn_user.id); hnSetLoggedIn(); hnRenderFeed(); } catch(e) {}
}
...
function hnSubmitPost() {
  ...
  hnSavePost_fb(post,
    function(){ ta.value=''; hnClearImg(); hnUpdateChar(); hnRenderFeed(); hnRenderSidebar(); btn.disabled=false; btn.textContent='Pubblica'; hnNotifMentions(text, post.id, text); hnNotifGroupTags(text, post.id, text); hn_poll_options=[]; hn_poll_deadline_val=''; var pc=document.getElementById('hn-poll-compose'); if(pc)pc.style['display']='none'; hn_link_url=''; hn_link_title=''; var lr=document.getElementById('hn-linkurl-row'); if(lr)lr.style['display']='none'; var li2=document.getElementById('hn-linkurl'); if(li2)li2.value=''; var lt2=document.getElementById('hn-linktitle'); if(lt2)lt2.value=''; },
...
function hnRenderFeed() {
  ...
  if (hn_posts.length === 0) {
    console.log('[render] called with empty hn_posts! data_ready='+hn_data_ready, new Error().stack);
  }
  var fp = hnFiltered();
...
function hnVotePoll(pid, opt_idx) {
  ...
  hnSavePost_fb(post,function(){hnRenderFeed();},function(){alert('Errore nel voto.');});
}
...
function hnSavePost(pid) {
  ...
  hnSavePost_fb(post_to_save, function(){ hnRenderFeed(); }, function(){ alert('Errore nel salvataggio.'); hnRenderFeed(); });
}
function hnDeletePost(pid) {
  if (!confirm('Eliminare questo post?')) return;
  for (var i = 0; i < hn_posts.length; i++) { if (hn_posts[i].id === pid) { hn_posts.splice(i,1); break; } }
  hnDeletePost_fb(pid, function(){ hnRenderFeed(); hnRenderSidebar(); }, function(){ alert('Errore nell\'eliminazione.'); hnRenderFeed(); });
}
