function hnGoHome() {
  ...
  hnGET(hn_api+'/'+hn_bin_post+'/latest',
    function(d) { hn_posts = (d&&d.record&&d.record.posts) ? d.record.posts : []; posts_loaded=true; tryRender(); },
    function() { posts_loaded=true; tryRender(); }
  );
  hnGET(hn_api+'/'+hn_bin_prof+'/latest',
    function(d) { hn_profiles = (d&&d.record&&d.record.profiles) ? d.record.profiles : []; profs_loaded=true; tryRender(); },
...
function hnCheckPollExpiry() {
  hnGET(hn_api+'/'+hn_bin_post+'/latest', function(d) {
...
function hnCheckPollExpiryRun() {
  ...
  if (changed) hnPUT(hn_api+'/'+hn_bin_post,{posts:hn_posts},function(){hnRenderFeed();},function(){});
}
...
function hnInit() {
  ...
  var brand_btn = document.getElementById('hn-brand');
  if (brand_btn) brand_btn.onclick = function() { hnGoHome(); };
  // Retry after DOM settles in case Forumfree injects late
  setTimeout(function() {
    var b = document.getElementById('hn-brand');
    if (b) b.onclick = function() { hnGoHome(); };
  }, 2000);
...
function hnRenderPost(p) {
  ...
  console.log('[dot] post by', p.authorHandle, '| prof found:', !!author_prof, '| last_seen:', author_prof ? author_prof.last_seen : 'N/A', '| online:', hnIsOnline(author_prof));
...
function hnVotePoll(pid, opt_idx) {
  ...
  hnPUT(hn_api+'/'+hn_bin_post,{posts:hn_posts},function(){hnRenderFeed();},function(){alert('Errore nel voto.');});
}
...
function hnNotifMentions(text, post_id, post_text) {
  console.log('[mention-notif] text='+text+' post_id='+post_id);
  var found = {};
  var i = 0;
  while (i < text.length) {
    if (text.charAt(i) === '@') {
...
function hnUpdatePresence() {
  ...
  hnPUT(hn_api+'/'+hn_bin_prof, {profiles:hn_profiles}, function(){}, function(){});
}
...
setInterval(function() {
  if (!hn_user) return;
  hnGET(hn_api+'/'+hn_bin_notif+'/latest',
    function(d) { hn_notifs = (d&&d.record&&d.record.notifications) ? d.record.notifications : []; hnUpdateBadge(); },
    function() {}
  );
  hnUpdatePresence();
  hnCheckPollExpiry();
  hnGET(hn_api+'/'+hn_bin_prof+'/latest',
    function(d) {
      hn_profiles = (d&&d.record&&d.record.profiles) ? d.record.profiles : [];
      hnRenderFeed();
      hnRenderSidebar();
    },
    function() { hnRenderFeed(); hnRenderSidebar(); }
  );
}, 60000);
hnUpdatePresence();
