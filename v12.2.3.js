...
function hnRenderSearchUsers(users) {
  ...
  for (var i=0;i<users.length&&i<5;i++) {
    ...
    html+='<div class="hn-mention-item" onclick="hnCloseSearch();hnOpenProfile(\''+u.id+'\')">'
      ...
      '<div class="hn-mention-name">'+hnEsc(u.name)+hnVerifiedBadge(u)+hnOnlineDot(u)+'</div>'+
...
function hnSubmitPost() {
  ...
  if (hn_poll_options.length >= 2) {
    ...
    if (valid_opts.length >= 2) poll_data = {options: valid_opts};
  }
...
function hnRenderPost(p) {
  ...
  if (p.poll) {
    ...
    for (var pi = 0; pi < p.poll.options.length; pi++) {
      ...
      pollHtml += '<div class="hn-poll-option'+(user_voted?' hn-poll-voted':'')+'" onclick="hnVotePoll(\''+p.id+'\','+pi+')">'+
  ...
  return '<div class="hn-post" id="hn-p-'+p.id+'"><div class="hn-phead">'+
    ...
    '<span class="hn-pname" onclick="hnOpenProfile(\''+p.authorId+'\')">'+hnEsc(p.authorName)+'</span>'+hnVerifiedBadge(author_prof)+hnOnlineDot(author_prof)+
...
function hnToggleDislike(pid) {
  ...
  hnPUT(hn_api+'/'+hn_bin_post,{posts:hn_posts},function(){hnRenderFeed();},function(){alert('Errore dislike.');});
}
