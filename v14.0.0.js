...
/* ── FIREBASE CONFIG ── */
var hn_fb_config = {
  apiKey: "AIzaSyAgCGRQKtB9cmyH4Qo7NBeu8QXQrdGtxAY",
  authDomain: "hunternet-7c5e5.firebaseapp.com",
  databaseURL: "https://hunternet-7c5e5-default-rtdb.europe-west1.firebasedatabase.app",
  projectId: "hunternet-7c5e5",
  storageBucket: "hunternet-7c5e5.firebasestorage.app",
  messagingSenderId: "496062993294",
  appId: "1:496062993294:web:bb5dc726e1666757348adf"
};
var hn_fb_app = null;
var hn_fb_db  = null;
...
function hnFbListenNotifs(uid) {
  hn_fb_ref('hunternet/notifications/'+uid).on('value', function(snap) {
    var d = snap.val();
    hn_notifs_mine = d ? Object.values(d) : [];
    hnUpdateBadge();
  });
}
...
function hnSavePosts(ok, fail) {
  ...
  hn_fb_ref('hunternet/posts').set(obj, ok, fail);
}
...
function hnSaveProfiles(ok, fail) {
  ...
  hn_fb_ref('hunternet/profiles').set(obj, ok, fail);
}
...
function hnSaveCreds(ok, fail) {
  ...
  hn_fb_ref('hunternet/credentials').set(obj, ok, fail);
}

/* Save single notif for a user */
function hnSaveNotif(notif, ok, fail) {
  hn_fb_ref('hunternet/notifications/'+notif.to_id+'/'+notif.id).set(notif, ok, fail);
}
...
function hnMarkNotifReadFb(to_id, nid) {
  hn_fb_ref('hunternet/notifications/'+to_id+'/'+nid+'/read').set(true);
}
...
function hnDoLogin() {
  ...
  hn_fb_get('hunternet/notifications/'+found.id, function(d) {
    hn_notifs = d ? Object.values(d) : [];
    hnUpdateBadge();
  }, function(){});
  hnFbListenNotifs(found.id);
  hnCloseModal('auth');
  hnSetLoggedIn();
  hnRenderFeed();
}
