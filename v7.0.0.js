...
function hnFormatText(text) {
  if (!text) return '';
  var t = text.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
  var re_b = new RegExp('\\*\\*([^*]+)\\*\\*','g'); t = t.replace(re_b,'<span class="hn-fmt-b">$1</span>');
  var re_s = new RegExp('~~([^~]+)~~','g'); t = t.replace(re_s,'<span class="hn-fmt-s">$1</span>');
  var re_u = new RegExp('__([^_]+)__','g'); t = t.replace(re_u,'<span class="hn-fmt-u">$1</span>');
  var re_i = new RegExp('_([^_]+)_','g'); t = t.replace(re_i,'<span class="hn-fmt-i">$1</span>');
...
function hnApplyFormat(fmt) {
  ...
  var wrap = '';
  if (fmt === 'b') wrap = '**';
  else if (fmt === 'i') wrap = '_';
  else if (fmt === 'u') wrap = '__';
  else if (fmt === 's') wrap = '~~';
  if (!wrap) return;
  var newText = before + wrap + (sel || '') + wrap + after;
  ta.value = newText;
  var cursor = start + wrap.length + (sel ? sel.length + wrap.length : wrap.length);
  ta.setSelectionRange(sel ? start + wrap.length : cursor, sel ? start + wrap.length + sel.length : cursor);
  ta.focus();
  hnUpdateChar();
}
