function hnGoHome() {
  ...
  if (list) list.innerHTML = '<div class="hn-msg"><div class="hn-spin"></div></div>';
  if (!hn_fb_db) { hnFbInit(function(){ hnGoHome(); }); return; }
  var posts_loaded = false;
  var profs_loaded = false;
  function tryRender() {
    if (!posts_loaded || !profs_loaded) return;
    hnRenderFeed(); hnRenderSidebar(); hnBuildHashtags();
  }
  hn_fb_get('hunternet/posts',
    function(d) { if(d){hn_posts=Object.values(d);hn_posts.sort(function(a,b){return new Date(b.createdAt)-new Date(a.createdAt);});}else{hn_posts=[];} posts_loaded=true; tryRender(); },
    function() { posts_loaded=true; tryRender(); }
  );
  hn_fb_get('hunternet/profiles',
    function(d) { hn_profiles = d ? Object.values(d) : []; profs_loaded=true; tryRender(); },
    function() { profs_loaded=true; tryRender(); }
  );
}


function hnCheckPollExpiry() {
  hn_fb_get('hunternet/posts', function(d) {
    hn_posts = (d&&d.record&&d.record.posts) ? d.record.posts : [];
...
var _hn_posts_real = [];
Object.defineProperty(window, 'hn_posts', {
  get: function() { return _hn_posts_real; },
  set: function(v) {
    if (_hn_posts_real.length > 0 && (!v || v.length === 0)) {
      console.error('[hn_posts EMPTIED]', new Error().stack);
    }
    _hn_posts_real = v;
  },
  configurable: true
...
function hnInit() {
  ...
  hn_fb_get('hunternet/profiles', function(d) {
    hn_profiles = d ? Object.values(d) : [];
    tryDone();
  }, function(){ hasErr=true; tryDone(); });
  hn_fb_get('hunternet/posts', function(d) {
    if (d) {
      hn_posts = Object.values(d);
      hn_posts.sort(function(a,b){ return new Date(b.createdAt)-new Date(a.createdAt); });
    } else { hn_posts = []; }
    tryDone();
  }, function(){ hasErr=true; tryDone(); });
...
function hnRestoreSession() {
  ...
  if (!s) return;
  try { hn_user = JSON.parse(s); hn_fb_get('hunternet/notifications/'+hn_user.id, function(d){ hn_notifs = d ? Object.values(d) : []; hnUpdateBadge(); }, function(){}); hnFbListenNotifs(hn_user.id); hnSetLoggedIn(); if (hn_data_ready) hnRenderFeed(); } catch(e) {}
}
...
function hnSubmitComment(pid) {
  ...
  if (!post) {
    hn_fb_get('hunternet/posts/'+pid, function(d) {
      if (d) { for(var x=0;x<hn_posts.length;x++){if(hn_posts[x].id===pid){return;}} hn_posts.push(d); }
    }, function(){});
    return;
  }
...
function hnDeleteNotif(nid) {
  for (var i = 0; i < hn_notifs.length; i++) {
    if (hn_notifs[i].id === nid) { hn_notifs.splice(i, 1); break; }
  }
  hnUpdateBadge();
  hnRenderNotifDrop();
  /* notif auto-saved via hnSaveNotif */
}
...
function hnNotifClick(nid, post_id, from_id, type) {
  ...
  if (type === 'follow') {
    ...
    hn_fb_get('hunternet/posts',
      function(d) {
        if(d){hn_posts=Object.values(d);hn_posts.sort(function(a,b){return new Date(b.createdAt)-new Date(a.createdAt);});}else{hn_posts=[];}
        if (hn_tab_cur !== 'all') {
          hn_tab_cur = 'all';
          var tabs = document.querySelectorAll('.hn-tab');
          for (var i=0;i<tabs.length;i++) tabs[i].classList.remove('hn-active');
...
function hnIsOnline(prof) {
  ...
  return (Date.now() - new Date(prof.last_seen).getTime()) < 180000;
}
...
/* Realtime listeners replace polling for posts/profiles/notifs.
   Only presence update remains periodic. */
