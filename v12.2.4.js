...
function hnSubmitPost() {
  ...
  hnPUT(hn_api+'/'+hn_bin_post, {posts:hn_posts},
    function(){ ta.value=''; hnClearImg(); hnUpdateChar(); hnRenderFeed(); hnRenderSidebar(); btn.disabled=false; btn.textContent='Pubblica'; hnNotifMentions(text, post.id, text); hnNotifGroupTags(text, post.id, text); hn_poll_options=[]; var pc=document.getElementById('hn-poll-compose'); if(pc)pc.style['display']='none'; },
...
function hnAddPollOpt() {
  if (hn_poll_options.length >= 4) return;
